package com.imooc.coupon.excutor.impl;

import com.alibaba.fastjson.JSON;
import com.imooc.coupon.constant.CouponCategory;
import com.imooc.coupon.constant.RuleFlag;
import com.imooc.coupon.excutor.AbstractExecutor;
import com.imooc.coupon.excutor.RuleExecutor;
...
public class ManJianZheKouExecutor extends AbstractExecutor implements RuleExecutor {
     ...
     * 需要注意:
     * 1. 这里实现的是 满减 + 折扣优惠券的校验
     * 2. 当使用多类优惠券时, 必须要所有的商品类型都包含在内, 即差集为空
     *
    ...
    protected boolean isGoodsTypeSatisfy(SettlementInfo settlement) {
        ...
        settlement.getCouponAndTemplateInfos().forEach(e -> {
            ...
            templateGoodsType.addAll(JSON.parseObject(
                    e.getTemplate().getRule().getUsage().getGoodsType(),
                    List.class
            ));

        });

        // 当使用多类优惠券时, 必须要所有的商品类型都包含在内, 即差集为空
    ...
    private boolean isTemplateCanShared(SettlementInfo.CouponAndTemplateInfo manJian,
        ...
        List<String> list = Arrays.asList(manjianKey, zhekouKey);
