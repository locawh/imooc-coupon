...
public class RedisServiceImpl implements IRedisService {
    ...
    public void saveEmptyCouponListToCache(Long userId, List<Integer> status) {
        ...
        SessionCallback<Object> sessionCallback = new SessionCallback<Object>() {
            ...
            public Object execute(RedisOperations operations) throws DataAccessException {
                status.forEach(e -> {
                    String redisKey = status2RedisKey(e, userId);
                });
                return null;
            }
        };
    ...
    /**
     * <h2>根据 userId 和 status 找到缓存的优惠券列表数据</h2>
     *
     * @param userId 用户 id
     * @param status 优惠券状态 {@link com.imooc.coupon.constant.CouponStatus}
     * @return {@link Coupon}s, 需要注意  可能会返回 null, 代表从没有过记录
     */
    @Override
    public List<Coupon> getCachedCoupons(Long userId, Integer status) {
        ...
        List<String> couponStrs = (List<String>) redisTemplate.opsForHash().values(redisKey)
                .stream()
    ...
    private Integer addCouponToCacheForExpired(Long userId, List<Coupon> coupons) throws CouponException {
        ...
        // status 是 EXPIRED, 代表是已有的优惠券过期了, 影响到两个 Cache  >>  USABLE, EXPIRED
    ...
    private Integer addCouponToCacheForUsed(Long userId, List<Coupon> coupons) throws CouponException {
        ...
        String redisKeyForUsed = status2RedisKey(CouponStatus.EXPIRED.getCode(), userId);
        //获取当前用户可用的优惠券
        ...
        coupons.forEach(e ->
                needCachedForUsed.put(e.getId().toString(), JSON.toJSONString(e))
        );
    ...
    private Integer addCouponToCacheForUsable(Long userId, List<Coupon> coupons) {
        ...
        coupons.forEach(e ->
                needCachedObject.put(e.getId().toString(), JSON.toJSONString(e))
        );
    ...
    private String status2RedisKey(Integer status, Long userId) {
        ...
        switch (couponStatus) {
            case USABLE:
                String.format("%s%s", Constant.RedisPrefix.USER_COUPON_USABLE, userId);
                break;
            case USED:
                String.format("%s%s", Constant.RedisPrefix.USER_COUPON_USED, userId);
                break;
            case EXPIRED:
                String.format("%s%s", Constant.RedisPrefix.USER_COUPON_EXPIRED, userId);
                break;
        }
